...
/*   Updated: 2022/07/19 05:20:10 by jubae            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	_wilcard_sort(char **s, int c, int (*m)(const char *, char *))
{
	int		i;
...
void	_set_match(t_list *lst, char *s, char **sort)
{
    ...
    if (cnt == 0)
        ft_lstadd_back(&lst, ft_lstnew(ft_strdup(_subst(s, '\001', '*'))));
}
void	_check(t_list *lst, char *s)
{
    ...
    free(temp1);
    _wilcard_sort(sort, cnt, my_strcmp);
