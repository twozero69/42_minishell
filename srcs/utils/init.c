...
/*   Updated: 2022/06/26 04:35:03 by younglee         ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
static void	my_dup(int *dst_fd, int src_fd, t_shell *shell)
{
	*dst_fd = dup(src_fd);
	if (*dst_fd == FAIL)
		exit_with_error("init.c: dup failed", shell);
}
		...
		if (equal != NULL)
		{
			*equal = '\0';
			if (add_new_env(*envp, equal + 1, shell) == FAIL)
				exit_with_error("init.c: malloc failed", shell);
			*equal = '=';
		}
		envp++;
	}
...
void	init(int argc, char **argv, char **envp, t_shell *shell)
{
	...
	if (argc != 1 || argv == NULL || envp == NULL)
		exit_with_error("argument error", shell);
